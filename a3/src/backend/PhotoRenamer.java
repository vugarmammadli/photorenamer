...
public class PhotoRenamer {
	...
	public static void main(String[] args) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			System.out.println("8. See name history of image");
			...
			while(option != 0){
				...
				if(option == 7){
					...
					try {
						Configuration.saveImageFiles(selectedImage);
					} catch (IOException e) {
						e.printStackTrace();
					}
				...
				if(option == 8){
					...
					System.out.println("Please enter id of image to see name history: ");
					int imageId = scan.nextInt();
